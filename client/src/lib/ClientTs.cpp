...
DWORD WINAPI ClientStart(LPVOID lpParameter)
{
	...
	char final_nick[50] = { "" };
	strcpy(final_nick, config->getLanguage());
	strcat(final_nick, "$");
	strcat(final_nick, config->getNick());
	strcat(final_nick, "$");
	/* Connect to server on localhost:9987 with nickname "client", no default channel, no default channel password and server password "secret" */
	if ((error = ts3client_startConnection(scHandlerID, identity, config->getServerAddress(), PORT, final_nick, NULL, "", "secret")) != ERROR_ok) {
...
DWORD WINAPI STT_THREAD(LPVOID lpParameter)
{
	...
	a:
		...
		if (trad = fopen("translate.txt", "r")) //if file exist read the string from nuance dragon service
		{
			...
			{
				wxString final = "\n" + wxString::FromAscii(Session::Instance()->getConfig()->getLanguage()) + "\n" + wxString::FromUTF8(translate_jar);
...
DWORD WINAPI TTS_THREAD(LPVOID lpParameter)
{
	while (1)
	{
		if (tts_flag == true)
		{
			speak((char*)Session::Instance()->getConfig()->getLanguage(), (char*)strSpeak.mb_str().data());
			Sleep(300);
			tts_flag = false;
		}
		Sleep(50);
