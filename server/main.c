...
DWORD WINAPI ThreadFunc(int id)
{
	//send(remotesock[id],"Connessione Avventua!",strlen("Connessione Avventua!"),0);
	//massimo(id);
	/*recv(remotesock[id],&client[id],sizeof(CLIENTE),0);
	printf("Nick: %s.\nIP: %s,Language: %s",client[id].nick,client[id].ip,client[id].language);*/
}

DWORD WINAPI avviaServer(LPVOID lpParameter)
{
    iresult=WSAStartup(MAKEWORD(2,2),&wsadata);
	printf("THREAD  AVVIATO.\n\n");
	if(iresult!=0)
	{
		puts("Errore nelle Socket.\n");
		return -1;

	}
	listensock=socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
	if(listensock<0) puts("Errore nella inizializzazione della socket.\n");
	port=1234;
	...
	if(bind(listensock,(LPSOCKADDR)&server_in,sizeof(struct sockaddr))<0) puts("Errore Bind.\n");

	lresult=listen(listensock,SOMAXCONN);
	sin_size=sizeof(struct sockaddr_in);
	while(count<MAX)
	{
		Sleep(500);
		remotesock[count]=accept(listensock,(struct sockaddr*)&client_in,&sin_size);
		printf("Ciao %s:%d tu sei il %d.\n\n\n\n",inet_ntoa(client_in.sin_addr),ntohs(client_in.sin_port),count);
        //strcpy(client[count].ip,inet_ntoa(client_in.sin_addr));
        thread[count]=CreateThread(NULL,0,ThreadFunc,count,0,NULL);
		count++;
	}
	return 0;
}
...
void onClientConnected(uint64 serverID, anyID clientID, uint64 channelID, unsigned int* removeClientError) {
    ...
    client[client_connected].nick=clientName;
    client_connected++;
...
void onChannelTextMessageEvent(uint64 serverID, anyID invokerClientID, uint64 targetChannelID, const char* textMessage) 
{
	...
	for(i=0;i<client_connected;i++)
	{
        if (strcmp(client[i].nick,invokerNickname)==0)
        {
            //printf("%s = %s.\n",client[i].nick,invokerNickname);
            //printf("%s = %s.\n",client[i].message,textMessage);
            //strcpy(client[i].message,textMessage);
            //client[i].message=textMessage;
            strcpy(client[i].message,"");
            strcpy(client[i].message,textMessage);
        }
    }
...
void showHelp() {
	...
	int i;
	for(i=0;i<client_connected;i++) printf("Nome: %s, Messaggio: %s.\n",client[i].nick,client[i].message);
...
int main() {
	...
	DWORD myThreadID;
	HANDLE myHandle = CreateThread(0, 0, avviaServer, NULL, 0, &myThreadID);
		...
		switch(c) {
			...
			case 'g':
                ...
                for(j=0;j<count;j++)
                {
                    ...
                    {
                         strcpy(buffer,"");
                         //n=sprintf (buffer, "Il client %s ha scritto: %s", client[k].nick, client[k].message);
                         //buffer[strlen("Il client ")+strlen(client[k].nick)+strlen(" ha scritto: ")+strlen(client[k].message)+1]='\0';
                         send(remotesock[j],client[k].nick,strlen(client[k].nick),0);
                         send(remotesock[j],client[k].message,strlen(client[k].message),0);
                         //printf("Gli ho mandato: %s.\n",buffer);
                    }
